...
const image_1 = require("../adapters/image");
...
async function generatePost(data) {
    ...
    if (!llmKey)
        throw new Error(`LLM key for ${topic.llmProvider} not found`);
    const apiKey = (0, crypto_1.decrypt)(llmKey.encryptedKey);
    const adapter = (0, llm_1.createLlmAdapter)(topic.llmProvider, apiKey);
        ...
        try {
            ...
            if (!FREE_IMAGE_PROVIDERS.includes(topic.imageProvider)) {
                ...
                if (!imageKey) {
                    ...
                    imgApiKey = (0, crypto_1.decrypt)(imageKey.encryptedKey);
                }
            }
            if (FREE_IMAGE_PROVIDERS.includes(topic.imageProvider) || imgApiKey) {
                const imgAdapter = (0, image_1.createImageAdapter)(topic.imageProvider, imgApiKey);
                ...
                const imgRes = await axios_1.default.get(rawUrl, { responseType: 'arraybuffer', timeout: 60000 });
