...
router.post('/', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const adapter = createSocialAdapter(platform, creds);
...
router.put('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const mergedCreds = creds
      ? { ...JSON.parse(decrypt(account.encryptedCreds)), ...creds }
      : JSON.parse(decrypt(account.encryptedCreds));
    const adapter = createSocialAdapter(account.platform, mergedCreds);
    const ok = await adapter.verify();
    if (!ok) { res.status(400).json({ error: 'Не удалось подключиться — проверь данные' }); return; }

    await prisma.socialAccount.update({
      where: { id: account.id },
      data: {
        ...(label ? { label } : {}),
        encryptedCreds: encrypt(JSON.stringify(mergedCreds)),
      },
    });
    res.json({ ok: true });
  } catch (e: any) {
